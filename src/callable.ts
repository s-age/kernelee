 ...
 * `KernelError` / `BufferError` (TS has no compile-time hook, so this class
 * of programming error throws at module evaluation / wire time, where the
 * stack names the offender):
 *
 * - `'duplicateSymbolId'` — two `defineCallable` calls minted the same
 *   `"prefix.method"` id. Left undetected, the second `wire` would collide
 *   in `KernelBuilder` — which would throw `'duplicate'` only on the cold
 *   path where both are actually wired; the mint site is earlier and always
 *   runs, so the collision is caught up front instead.
 * - `'reservedMethodName'` — a spec key that collides with the generated
 *   surface (`wire`, `__spec`): both live alongside the generated symbols on
 *   the same object, so a spec key with either name would collide.
 ...
 * {@link portKV}.
 *
 * The 2×2 (leaf/composing × value/verb) is declared explicitly via a marker,
 * since TS cannot read either axis from a type at runtime — which is also
 ...
 * (TS cannot read a JSDoc comment at runtime, so the doc is an argument).
 * Omitted or blank, `defineCallable` warns (rather than throws) and the
 * symbol carries no description.
 */
 ...
 * back into the mesh, so the kernel is handed in at call time.
 */
 ...
 * `registerVerb`. TS splits `register`/`registerVerb` by name (rather than
 * overloading on return type), so the split surfaces here as an explicit
 * marker.
 */
 ...
 * {@link portV}, completing the 2×2 (leaf/composing × value/verb).
 */
 ...
 * with *extra* keys pass silently — and a typo'd key *is* an extra key plus
 * a missing one, so without this guard only the missing half would error.
 * Every key of `D` beyond the spec is forced to `never`, so an extra method
 * fails to type-check even on a non-fresh (already-typed) object where
 * excess-property checking wouldn't fire.
 */
 ...
 * it. Only `defineCallable`-minted ids are checked here; hand-minted
 * `symbol(id)` calls are not in this ledger. A collision
 ...
 *
 * A re-claim by the *same* prefix also throws: a TS module evaluates once,
 * so a second `defineCallable` with the same prefix+method can only be a
 * mistake.
 */
 ...
 *    `LifePort.xxx` symbols.
 * 3. **wire** — one `register` per spec key is generated, so no hand-written
 *    id list exists to drift.
 *
 * Failure modes at mint time (module evaluation):
 * - a cross-definition id collision throws `CallableError`
 *   (`'duplicateSymbolId'`);
 * - a missing/blank `doc` warns via `console.warn` (not a throw), and the
 *   symbol then carries no description (blank in the wiring graph);
